             ...
             switch (c)
             {
               ...
               case 'b':
    	            ...
    	            while ( i < end )
    	            {
                          ...
                          while (Abs(read[1] - InputPosition) > 3 || Abs(read1[1] - InputPosition1) > 3 || Abs(read2[1] - InputPosition2) > 3 )
						  {
							com3_ServoTRX(send, 4, read, 2);
							com3_ServoTRX(send1, 4, read1, 2);
							com3_ServoTRX(send2, 4, read2, 2);
							printf("Servo1 read data = %d\n\n", read[1]);
							printf("Servo2 read data = %d\n\n", read1[1]);
							printf("Servo3 read data = %d\n\n", read2[1]);
						  }
						  ...
						  //if (max >= 80) sleep_time = (int)(max * PauseTimeLongConstant[Speed]);
                          //else sleep_time = (int)(max * PauseTimeConstant[Speed]);
...
int MaxFind(int index)
{
    ...
    {
         max = Abs(RobotMotion[index].command[1] -  RobotMotion[index - 4].command[1]);         
         while (j <= 2)
         {
               temp = Abs(RobotMotion[index + j].command[1] -  RobotMotion[index + j - 4].command[1]);        
               if (temp > max) max = temp;        
               j = j + 1;   
         }      
    }
